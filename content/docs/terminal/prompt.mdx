...
Nermalputs an editor at the shell prompt. Nothing to install, no plugin to source
...
<Frame caption="Placeholder — video: typing a command with a ghost suggestion, accepting it, then opening the completion menu">
  <img src="/images/placeholder.svg" alt="The Nermalprompt" />
</Frame>
...
When Nermalhas nothing useful to offer, the <kbd>⇥</kbd> falls through to your
...
readout, the "command finished" notification, and `Nermalprocs` are built on.
...
```zsh
# ~/.zshrc — works at a Nermalprompt with the editor off
...
Settings while it is off — both are menus Nermalopens inside that editor, so
...
branch readout, `Nermalprocs`. The setting moves the line editor, not the
integration.
