...
another. Nermalgives that loop a primitive instead of leaving it to screen
scraping.
...
# 2. hand it a task
Nermalsend "$PANE" 'claude -p "add tests for the parser"' --enter

# 3. sleep until it needs you or finishes
Nermalwait "$PANE" --until waiting,done --changed --timeout 600
...
```bash
Nermalwait [%PANE] [--until STATE,…] [--changed] [--timeout SECS] [--interval MS]
```
...
Nermalwait "$PANE" --until free --changed --timeout 900
...
<Warning>
  ...
  own root process *is* the command — what `Nermalrun` spawns — looks free the
  whole time it runs; wait on `Nermalrun` itself instead, it already blocks. And
...
```bash
Nermalwait "$PANE" --until waiting --changed   # it needs something
Nermalcapture "$PANE" --plain | tail -20       # see what it is asking
Nermalsend "$PANE" --key down --key enter      # answer it
Nermalsend "$PANE" --key C-c                   # or stop it altogether
```
...
If `Nermalwait` times out and `Nermalagents` shows a status that never changes,
the likely cause is that the agent's status hooks are not installed or are out
of date — the agent is working fine, it just has no way to say so. `Nermaldoctor`
reports where every agent's hooks stand, and `Nermalagents` names the specific
one when it can see the gap.

## Watching everything at once

```bash
Nermalagents          # every agent on the machine: pane, agent, status, message
Nermalagents --json   # the same, parseable
```
...
Nermalinstalls nothing into `~/.claude` for it — no switch in **Settings →
...
- **Never `send` into a pane you did not open.** Check `Nermalagents` first.
- **Never close a pane, tab, or workspace you did not create.**
- **Never `server stop` or `server restart`.** Every pane on the machine dies
  with it.
- **Clean up what you did create** — `Nermalpane close %83` when you are done.
  An interrupted `run` leaves its pane behind; `Nermalpane ls --all` shows those,
  and `Nermalpane close --orphans` clears them. That last one is a human's
