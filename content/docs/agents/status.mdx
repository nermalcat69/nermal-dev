...
waiting for permission look identical from outside. Nermalfixes that by letting
...
<Note>
  ...
  carries no status dot and `Nermalwait` times out rather than returning.
</Note>
...
The same three states are what `Nermalagents` reports as `working` / `waiting` /
`done`, and what [`Nermalwait`](/agents/orchestration) blocks on. A fourth state,
...
Nermalkeeps a status item in the system tray (menu bar on macOS). It flips to an
...
If nothing recognisable is running, Nermalsays *"No running coding agent found"*
