---
title: "Coding agents"
description: "What Nermaldoes around Claude Code, Codex, TraeCode, and 19 others — without ever wrapping them."
---

Nermalrecognises coding agents running in a pane and builds around them. It does
not wrap them, proxy them, or replace their interface: the agent you start is
the agent you get, running in a normal PTY, with its own UI. Nermaladds the
...
<Frame caption="Placeholder — screenshot: a sidebar of agent sessions across several repos, each with a brand avatar and status dot">
  <img src="/images/placeholder.svg" alt="Agent sessions in the Nermalsidebar" />
</Frame>
...
itself, over a channel Nermalinstalls into that agent's configuration. It powers
the status dots, the notifications, the tray icon, and `Nermalwait`.
...
<CardGroup cols={3}>
  ...
  <Card title="Orchestration" icon="diagram-project" href="/agents/orchestration">
    One agent driving another with `Nermalwait`.
  </Card>
</CardGroup>
