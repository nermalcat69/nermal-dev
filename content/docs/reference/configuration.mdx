---
title: "config.json"
description: "Every key Nermalreads, its type, and its default."
---

| | |
|---|---|
| macOS / Linux | `~/.config/Nermal/config.json` |
| Windows | `%APPDATA%\Nermal\config.json` |
...
| Key | Type | Default | |
|---|---|---|---|
| `prompt_editor` | bool | `true` | Nermaledits the shell prompt. Off sends every key at the prompt to the shell, so its own line editor (ZLE, readline) owns editing. Shell integration stays on. |
...
<Note>
  If the file cannot be parsed, Nermalstarts on defaults, keeps your original at
