...
description: "What macOS asks you, why, and what Nermalitself holds."
---

## Why macOS asks Nermalfor permission
...
If Nermaldeclared no usage strings, that request would be **denied outright with
no prompt at all**, and the program would look broken for no visible reason.

So Nermaldeclares the matching usage strings, and you get the normal one-time
prompt:
...
| | |
|---|---|
| Settings, themes, window state | `~/.config/Nermal/` (`%APPDATA%\Nermal\` on Windows) |
