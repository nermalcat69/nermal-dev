...
description: "What Nermalinjects into your shell, and what it buys you."
---
...
| Window title | `OSC 0` | Tab labels. Only PowerShell is given this — zsh, bash, and fish already set a title of their own, and Nermalreads whatever they emit |

## What turns off without it

Run a shell Nermaldoes not integrate with, and everything below still works —
...
available everywhere. Where Nermalcan see the pane's processes it reads the
...
To get integration into that shell, make it the shell Nermalstarts:

- **A remote host.** Nermalasks the host for `$SHELL` when it connects and
  ...
  your `.bashrc`: it replaces the process Nermalset up.
