...
- Check **Settings → Agents → Install the Nermalcommand on PATH** is on.
...
Inside a Nermalpane it works regardless, since panes inherit the app's
environment.

## The server is unreachable

`Nermaldoctor` says so, and the GUI cannot open panes.

Start it with `Nermalserver start`. If you are an agent or a script,
...
unavoidable. The *screens* should come back: Nermalkeeps a capped tail of each
...
Nermalupdated in place, so the app is new and your panes are still served by the
...
|---|---|
| *"running an old Nermalserver that this copy cannot talk to"* | The server there predates your client's protocol. Let Nermalupdate it — this ends every session on that machine. |
| *"running a newer Nermalserver than this copy"* | Update Nermalhere instead, or replace the server there. |
| *"answered, but not as a Nermalserver"* | Something else is listening, or the binary is not what Nermalexpects. |
| *"Nermalno longer has a way to reach &lt;machine&gt;"* | The SSH link dropped. Reconnect from the switcher. |
...
The usual cause is a shell you started by hand. Nermalinjects its
...
**Make it the shell Nermalstarts.** On the remote host:

```sh
chsh -s /bin/zsh      # then reconnect
```

Nermalasks the host for `$SHELL` at connect time and bootstraps *that* shell,
...
`.bashrc`, does not help: both run after (or instead of) the shell Nermalset
up.
...
- **The shell is not one Nermalintegrates.** It injects into zsh, bash, fish,
...
Where Nermalcan see the pane's processes it falls back to reading the
...
If the file cannot be parsed, Nermalstarts on defaults and keeps your original at
...
## `Nermalcapture … | head -1` printed a Rust panic
...
<CardGroup cols={2}>
  ...
  <Card title="Report an issue" icon="github" href="https://github.com/Nermalcat69/Nermal-ide/issues/new">
    Include `Nermaldoctor` output and your platform.
  </Card>
</CardGroup>
