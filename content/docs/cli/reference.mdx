...
Set inside every Nermalpane, inherited by anything launched from one.
...
Outside a Nermalshell, address-taking verbs fail with
`not inside a Nermalshell — pass an explicit %pane/@tab/workspace`.
...
| `124` | `Nermalwait` timed out (the `timeout(1)` convention) |
...
| *other* | Only from `Nermalrun`, which passes the child's exit code through |
...
### `Nermalrun [--keep] [--cwd DIR] [--ws WORKSPACE] -- CMD...`
...
### `Nermalsend [%PANE] [TEXT] [--enter] [--key KEY]…`
...
none, so `Nermalsend %42 --enter` runs whatever pane 42 already has typed. With
...
to do. `--enter` is that key only for the `%`-marked spelling: `Nermalsend 83
...
type an address-shaped string, name the pane as well: `Nermalsend %42 %3x`.
...
### `Nermalcapture [%PANE] [--plain] [--scrollback] [--tail N]`
...
nothing, so `Nermalagents` shows it standing still and `Nermalwait` sits there
...
prefix `Nermalls` prints). An ambiguous name or prefix is an error that lists the
candidates.
...
Prefer `Nermalnew <path>` over `ws new` when you want something usable: `ws new`
leaves an empty workspace you then have to populate, while `Nermalnew --json`
...
| Command | Effect |
|---|---|
| `server status` | Same as `Nermalstatus` |
