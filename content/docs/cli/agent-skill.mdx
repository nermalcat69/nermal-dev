...
description: "Teaching a coding agent to use Nermalproperly — including when not to."
---
...
<Note>
  This is the only skill Nermalhas — nothing in **Settings → Agents** installs
  one for you. Using the CLI to run *other* agents — a worker pane, `Nermalwait`,
...
- **Never `send` into a pane you did not open.** Keystrokes land in the middle
  of whatever is happening there. Check `Nermalagents` first.
...
- **Never `Nermalserver start` on your own initiative** when `doctor` says the
  server is unreachable — starting one the user did not ask for changes what
  their GUI attaches to. Tell them instead.
- **Clean up what you did create.** `Nermalpane close %83` when the scratch pane
  is done with.
...
Nermalwait %3 --until waiting,done --changed --timeout 600
```
...
command actually started, babysit with `Nermalwait`, collect the result out of
...
shortest description of how to use Nermalas a job runner. Start with the
