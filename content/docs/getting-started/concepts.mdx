...
is the only thing in Nermalthat actually runs something.
...
label is the best evidence Nermalhas: a name you set, else the coding agent
...
Workspaces are how Nermalkeeps ten repositories from becoming forty
...
<Frame caption="Placeholder — screenshot: the workspace switcher, workspaces on the left, that workspace's tabs on the right">
  <img src="/images/placeholder.svg" alt="The Nermalworkspace switcher" />
</Frame>
...
Quitting Nermalcloses the window and leaves that server running. Your build keeps
building, your agent keeps working, your SSH session stays up. Open Nermalagain
...
|---|---|
| `Nermal_PANE` | This pane's id — the default target of `Nermalsplit`, `send`, `capture`, `procs`. |
| `Nermal_WS` | This pane's workspace id. |
| `Nermal_CONFIG_DIR` | The config directory, which is how the CLI finds the right server. |

`echo $Nermal_PANE` is the fastest way to tell whether you are inside Nermalat all.
