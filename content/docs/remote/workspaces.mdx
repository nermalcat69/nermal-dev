...
further: Nermalruns a server on the far machine, and the whole workbench points
...
<Steps>
  ...
  <Step title="Pick a machine">
    Nermalconnects over the same SSH stack as everything else, so profiles,
  ...
  <Step title="Approve the server install, once">
    ...
    > Nermalwill write its server binary to *devbox* so this machine can host
...
that disagree never quietly half-work — Nermalinstalls the matching one instead.
...
<AccordionGroup>
  <Accordion title="Update Nermal's server on “devbox”?">
    ...
    cannot speak. Nermalhas already installed a matching server, but the one
...
<Warning>
  Both of these end other people's work if the machine is shared. Nermalspells
...
- **Fork an agent session.** The fork command would run against the *local*
  agent, so Nermaldoes not offer it.
...
```bash
Nermalmachine ls              # this machine plus every link the server holds
